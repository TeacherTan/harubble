...
> 仅面向未来或尚未完成的后端阶段规划。
>
> **状态说明（2026-04）**：本文档中的 Phase 11 / Phase 12 主要用于描述尚未完成的增强路线，不等于“当前后端完全缺失这些能力”。例如库内搜索的基础 MVP 已经落地，后续内容主要描述搜索增强方向。
>
> 已完成的后端能力（Phase 1~10）参见 [backend-completed-phases.md](./backend-completed-phases.md)。
>
...
2. **Phase 12B 剩余增强**：当前搜索已具备 `intro` / `belong` 命中表达与拼音召回，后续重点转向更细的召回质量、排序策略与中文输入体验打磨。
3. **Phase 12C** 歌词全文检索与索引维护：当前仍未进入实现，后续再补齐歌词 cache owner、状态模型与索引更新策略。
...
- `crates/harubble-core/src/download/service.rs`
- `crates/harubble-core/src/download/model.rs`
- `src-tauri/src/commands/downloads.rs`
- `src/lib/api.ts`
- `src/lib/types.ts`
- `docs/reference/backend-api-contract.md`
...
- 后端已注册 `search_library` Tauri command
- `AppState` 已挂载 `LibrarySearchService`
- 前端 `src/lib/api.ts` 与 `src/lib/types.ts` 已对齐搜索请求 / 响应类型
- 前端 `features/library/controller.svelte.ts` 已接入搜索查询、scope 切换、结果定位与 `indexState` 处理

因此本章描述的是 **已落地搜索 MVP 之上的增强路线**，而不是“从零开始引入搜索”。
...
2. **契约先行**：每一阶段都先在 `docs/reference/backend-api-contract.md` 冻结请求 / 响应模型，再推进 Rust 实现和前端接入。
...
- `crates/harubble-core/src/search/mod.rs` — 模块导出
- `crates/harubble-core/src/search/model.rs` — 搜索请求、结果项、命中字段、内部文档模型
- `crates/harubble-core/src/search/normalizer.rs` — 大小写、空白、标点、全半角等归一化
- `crates/harubble-core/src/search/index.rs` — Tantivy schema、索引构建、writer / reader 管理
- `crates/harubble-core/src/search/service.rs` — 查询执行、排序、结果映射、索引重建入口
...
- `crates/harubble-core/src/search/`
- `src/lib/types.ts`
- `src/lib/api.ts`
- `docs/reference/backend-api-contract.md`
