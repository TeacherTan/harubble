...
- `TagEntry { dimension: string; values: string[]; colors?: (string | null)[] }` — 已本地化的 tag 条目，附加在 `Album`、`AlbumDetail`、`SongEntry`、`SongDetail` 上；`colors` 与 `values` 索引一一对齐，表示每个 tag 值的可选颜色（hex 色值），无颜色时整个字段省略
